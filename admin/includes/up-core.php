...
if(isset($_POST['up_series'])){
	...
	if($in){
		header('location:'.$url.$resss_row['a_type2']."-anime/".str_replace(" ","-",strtolower($_POST['title'])));
	}
}
//end update series
...
if(isset($_POST['up_episode'])){
	...
	if(isset($_POST['reset'])) $date=time(); else $date=GetSQLValueString($_POST['c_date'],"int");
	
	$in=$ob->up_data("an_episodes","e_title=$title,e_subdub=$subdub,e_show=$show,e_not_yeird=$yeird,e_raw=$raw,e_hd=$hd,e_mirror1=$mirror1,e_mirror2=$mirror2,e_mirror3=$mirror3,e_mirror4=$mirror4,e_series=$series,e_date=$date,e_date2=$date2,e_coming_date=$coming_date","e_id=".$resss_row['e_id']);
...
if(isset($_POST['up_page'])){
	...
	$in=$ob->up_data("an_pages","p_title=$title,p_content=$content,p_link=$link,p_order=$order,p_position=$position","p_id=".$resss_row['p_id']);
	if($in){
...
if(isset($_POST['up_image'])){
	...
	$filename=rand(00000000,99999999).'_'.$_FILES['img_file']['name']; 
     move_uploaded_file($_FILES['img_file']['tmp_name'],"/home/daiky/public_html/images/".$filename);
        @chmod("/home/daiky/public_html/images/".$filename,0644);
	$filename=GetSQLValueString($filename,"text");
	if(file_exists("/home/daiky/public_html/images/".$_POST['c_image'])) unlink("../images/".$_POST['c_image']);
	}
	else{$filename=GetSQLValueString($_POST['c_image'],"text");}
    
	$in=$ob->up_data("an_images","i_bigtitle=$bigtitle,i_smalltitle=$smalltitle,i_link=$link,i_desc=$desc,i_file=$filename,i_date='$date'","i_id=".$id);
	if($in){
