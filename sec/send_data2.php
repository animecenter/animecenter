...
$check=mysql_fetch_assoc($ob->get_table("an_f_data","d_id=1"));
  if(isset($check) and $check['d_e_add']!=''){
  $episodes_arr=array();
  $episodes=mysql_query("SELECT *,DATE_FORMAT(FROM_UNIXTIME(`e_date`), '%Y-%m-%d %H:%i:%s') as d1,
  DATE_FORMAT(FROM_UNIXTIME(`e_date2`), '%Y-%m-%d %H:%i:%s') as d2 FROM `an_episodes` where e_id in (".substr($check['d_e_add'], 0, -1).")",$config);
  while($e=mysql_fetch_assoc($episodes)){
  $episodes_arr[]=$e;
  }
  
  $ob->up_data("an_f_data","d_e_add=''","d_id=1");
...
$check=mysql_fetch_assoc($ob->get_table("an_r_data","d_id=1"));
  ...
  $ob->up_data("an_r_data","d_e_add=''","d_id=1");
