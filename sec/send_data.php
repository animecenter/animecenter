...
$check=mysql_fetch_assoc($ob->get_table("an_f_data","d_id=1"));
  if(isset($check) and $check['d_s_add']!=''){
  $series_arr=array();
  $series=$ob->get_table("an_series","a_id in (".substr($check['d_s_add'], 0, -1).")");
  while($s=mysql_fetch_assoc($series)){
  $series_arr[]=$s;
  }
  
  $ob->up_data("an_f_data","d_s_add=''","d_id=1");
...
$check=mysql_fetch_assoc($ob->get_table("an_r_data","d_id=1"));
  ...
  $ob->up_data("an_r_data","d_s_add=''","d_id=1");
