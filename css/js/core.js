$(document).ready(function (e) {
    ...
    $("input.add-user").click(function (e) {
        ...
        if (pass1 != pass2 || pass1.length == 0) {
            parent.find("input[name='password']").css("border-color", "#f00");
            parent.find("input[name='re_password']").css("border-color", "#f00");
            err = 1;
        }
        else {
            parent.find("input[name='password']").css("border-color", "#B9B9B9");
            parent.find("input[name='re_password']").css("border-color", "#B9B9B9");
        }

        var loc = parent.find("input[name='email']").val().indexOf("@");
        if (loc < 0) {
            parent.find("input[name='email']").css("border-color", "#f00");
            err = 1;
        }
        if (err == 0) {
    ...
    $("body").delegate("#add-animi a.edit_anime_btn", "click", function () {
        ...
        $.post("./includes/edit_animelist.php", {
            list_id: list_id,
            status: status,
            watched: watched,
            score: score,
    ...
    $("body").delegate("#profileLinks a.rej_friend", "click", function () {
        var id = $(this).attr("val");
        $.post("req-up.php", {id: id, type: 'del2'}, function (data) {
            location.reload();
        });
    });
    /*-----------------------------------------*/
    $("iframe[width=0]").each(function () {
        $(this).remove();
    });
});