...
if(isset($_POST['id']) and $_POST['id']!=NULL and $_POST['num']>0 and isset($_SESSION['u_id'])){
	...
	$last_episode=mysql_fetch_assoc($ob->get_table("an_episodes","e_order=(select max(e_order) from an_episodes where e_series=".$id.")"));
    ...
    $order=$next_id;
	$in=$ob->insert_data("an_episodes(e_title,e_not_yeird,e_series,e_date,e_date2,e_order)","$title,$con,$id,$date,$date,$order");
