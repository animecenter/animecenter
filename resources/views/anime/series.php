...
if ((isset($_GET['id']) and $_GET['id'] != null)) {
    ...
    $last_episodes = mysql_fetch_assoc($ob->get_table("an_episodes",
    "e_series=" . $id . " and ISNULL(e_not_yeird) order by e_id DESC"));
...
<div id="wrap">
    ...
    ?>
        <div id="left_content">
            <div class="sec_top_one">
                <div class="fb-like" style="height:30px;"
                     data-href="<?php echo $url . $series_content['a_type2'] . "-anime/" . str_replace(" ", "-",
                             strtolower($series_content['a_title']));
    ?>" data-width="450" data-show-faces="false"
                     data-send="true"></div>
                <?php if (isset($_SESSION['u_username'])) {
    ?>
                    <a href="<?php echo $url;
    ?>admin/index.php?page=series_up&id=<?php echo $series_content['a_id'];
    ?>"
    ...
    ?>
                                <select class="member-select episodess">
                                    ...
                                    <option
                                        value="<?php echo $url;
    ?>admin/index.php?page=episode_add&id=<?php echo $series_content['a_id'];
    ?>">
                                        Add Episode Manually
                                    </option>
                                    <option value="<?php echo $url;
    ?>episode_auto_add.php"
                                            val="<?php echo $series_content['a_id'];
    ...
    ?>
                    ...
                    <div class="content">
                        <div class="categories">
                            <a href="#"
                               class="<?php echo $series_content['a_type2'];
    ?>">English <?php echo $series_content['a_type2'];
    ?></a>
                        </div>
                        <div class="img"><img src="<?php echo $url;
    ?>images/<?php echo $series_content['a_image'];
    ?>"/>
                        </div>
    ...
    ?>
                                <div class="text"><span>Genres:</span><?php echo str_replace(",", ", ",
                                        $series_content['a_genres']);
    ?></div>
                                <div class="text"><span>Episodes:</span> <?php echo $series_content['a_episodes'];
    ?>
                                </div>
                                <div class="text"><span>Type:</span> <?php echo $series_content['a_type'];
    ?></div>
                                <?php if ($series_content['a_prequel'] != null) {
    $ser_ser = explode(",", $series_content['a_prequel']);
    $ser_row = mysql_fetch_assoc($ob->get_table("an_series", "a_id=" . $ser_ser[0]));
    $link = $url . $ser_row['a_type2'] . "-anime/" . str_replace(" ", "-",
    ...
    ?>
                                <?php if ($series_content['a_sequel'] != null) {
    ...
    ?>
                                <?php if ($series_content['a_story'] != null) {
    ...
    ?>
                                <?php if ($series_content['a_side_story'] != null) {
    ...
    ?>
                                <?php if ($series_content['a_spin_off'] != null) {
    ...
    ?>
                                <?php if ($series_content['a_alternative'] != null) {
    ...
    ?>
                                <?php if ($series_content['a_other'] != null) {
    ...
    ?>
                                <div class="text age"><span>Age Permission: </span> <span class="age"
                                                                                          style="background:<?php echo $color;
    ?>"><?php echo $series_content['a_age'];
    ?></span>
                                </div>
                                <div class="text">
                                    <span>Plot Summary:</span> <?php echo $series_content['a_description'];
    ?></div>
                                <div class="text alternative">
                                    <span>Alternative Titles:</span> <?php echo $series_content['a_alternative_title'];
    ?>
                                </div>
                            <?php
    ...
    $link = $url . $options[4]['o_value'] . str_replace(" ", "-",
                                    strtolower($last_episodes['e_title']));
    ?>
                            <div class="latest-episode">
                                <a href="<?php echo $link;
    ?>">
                                    <div class="row main">Latest Episode</div>
                                    <div class="row sec"><?php echo str_replace($series_content['a_title'], "",
                                            $last_episodes['e_title']);
    ?></div>
    ...
    ?>
                ...
                <div class="rating_div">
                    <div class="views_value  view_series" id="<?=$series_content['a_id'];
    ?>"></div>
                    <div id='rateContainor' style='float: left; width: 200px; margin-left: 20px;'>
                        <div style='float:left;' value="<?php echo $series_content['a_rating'];
    ?>" id="rateDiv"
                             class='rating'></div>
                        <div style='float: left; font-size: 8pt; clear: both; width: 100%; display:none'
                             id='hint'></div>
                        <div id='hint2'
                             style='float:left;font-size:8pt'>   <?php echo "Average: " . (($series_content['a_rating']) ? sprintf("%.2f",
                                    $series_content['a_rating']) : 0) . " ( " . (($series_content['a_votes']) ? $series_content['a_votes'] : 0) . " votes)" ?> </div>
                    </div>
                </div>


                <div class="title" style="margin-bottom:5px;"><?php echo $series_content['a_title'];
    ?> Episodes</div>
                <div class="episodes">
                    <ul>
                        <?php
                        while ($episode = mysql_fetch_assoc($episodes)) {
                            $link = $url . $options[4]['o_value'] . str_replace(" ", "-",
                                    strtolower($episode['e_title']));
                            ?>
                            <li class="leaf"><a href="<?php echo $link;
                            ?>">
                                    <?php echo $episode['e_title'];
                            ?></a>
                                <?php if ($episode['e_not_yeird'] != null) {
    echo "<span>Not Yet Aired</span>";
}
                            ?></li>
                        <?php
        ...
        <!--/left_content-->
        <div id="right_content"><?php include_once("sidebar.php");
    ?></div>
    </div>
    <!--/content-->
    <script>
    ...
    ?>,
                starHalf: '<?php echo $url;
    ?>/images/star-half.png',
                starOff: '<?php echo $url;
    ?>/images/star-off.png',
                starOn: '<?php echo $url;
    ?>/images/star-on.png',
                starHover: '<?php echo $url;
    ?>/images/star-hover.png',
                target: ("#hint"),
                click: function (score, evt) {
                    ...
                    $.post("<?php echo $url;
    ?>rate-up.php", {
                        sid:<?php echo $series_content['a_id'];
    ?>,
                        rate: score
                    }, function (data) {
