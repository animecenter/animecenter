...
if (isset($_POST['add_series'])) {
    ...
    $in = $ob->insert_data("an_series(a_title,a_genres,a_episodes,a_type,a_age,a_type2,a_status,a_prequel,a_sequel,a_story,a_side_story,a_spin_off,a_alternative,a_other,a_position,a_description,a_alternative_title,a_image,a_date,a_date2)",
        "$title,$genres,$episodes,$type,$age,$type2,$status,$prequel,$sequel,$story,$s_story,$spin_off,$alternative_2,$other,'$position',$description,$alternative,'$filename',$date,$date");
...
if (isset($_POST['add_episode'])) {
    ...
    $date = time();
    $in = $ob->insert_data("an_episodes(e_title,e_subdub,e_show,e_not_yeird,e_raw,e_hd,e_mirror1,e_mirror2,e_mirror3,e_mirror4,e_series,e_date,e_date2,e_order,e_coming_date)",
        "$title,$subdub,$show,$yeird,$raw,$hd,$mirror1,$mirror2,$mirror3,$mirror4,$series,$date,$date,$order,$coming_date");
    $insert_id = mysql_insert_id();
...
if (isset($_POST['add_page'])) {
    ...
    $in = $ob->insert_data("an_pages(p_title,p_content,p_link,p_order,p_position)",
        "$title,$content,$link,$order,$position");
    if ($in) {
...
if (isset($_POST['add_image'])) {
    ...
    $in = $ob->insert_data("an_images(i_bigtitle,i_smalltitle,i_link,i_desc,i_file,i_date)",
        "$bigtitle,$smalltitle,$link,$desc,'$filename','$date'");
    if ($in) {
