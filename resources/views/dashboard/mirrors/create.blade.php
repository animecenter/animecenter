        ...
        <div class="col-xs-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    @endif
                    <form role="form" method="post" action="{{ url('dashboard/mirrors/create') }}">
                        {!! csrf_field() !!}
                        <div class="form-group">
                            <select name="user_id" class="form-control">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{
                                        old('user_id') === $user->id ? 'selected' : '' }}>{{ $user->username }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <select name="episode_id" class="form-control">
                                @foreach ($episodes as $episode)
                                    <option value="{{ $episode->id }}" {{ old('episode_id') === $episode->id ?
                                        'selected' : '' }}>{{ $episode->animeTitle . ' ' . $episode->number }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <select name="mirror_source_id" class="form-control">
                                @foreach ($mirrorSources as $mirrorSource)
                                    <option value="{{ $mirrorSource->id }}" {{
                                        old('mirror_source_id') === $mirrorSource->id ? 'selected' : ''
                        ...
                        <div class="form-group">
                            <select name="translation" class="form-control">
                                <option value="subbed" {{ old('translation') === 'subbed' ? 'selected' : '' }}>Subbed</option>
                                <option value="dubbed" {{ old('translation') === 'dubbed' ? 'selected' : '' }}>Dubbed</option>
                                <option value="raw" {{ old('translation') === 'raw' ? 'selected' : '' }}>Raw</option>
                            </select>
                        </div>
                        <div class="form-group">
