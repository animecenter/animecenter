...
require_once("index-core.php");
		if(strpos($_SERVER['REQUEST_URI'],$options[2]['o_value']) or strpos($_SERVER['REQUEST_URI'],$options[3]['o_value'])){
			if((isset($_GET['id']) and $_GET['id']!=NULL)){
				...
				if(mysql_num_rows($sss)<=0) header("location:".$url."404Page");
		...
		elseif(strpos($_SERVER['REQUEST_URI'],$options[4]['o_value'])){
		  if((isset($_GET['id']) and $_GET['id']!=NULL)){
				...
				if(mysql_num_rows($episode)<=0) header("location:".$url."404Page");
...
<div id="header">
    <div id="header_cont">
		...
		else echo '<div class="text">Watch Anime in <span> English Subbed-Dubbed </span> Online for Free</div>';
		 ?>
        </div>
        <div id="nav">
            <ul>
			...
			if(isset($top_page['p_link']) and $top_page['p_link']!=NULL) $link=$top_page['p_link'];
			else $link=$url."page.php?page_id=".$top_page['p_id'];
			if($top_page['p_title']=="Anime list"){$yes=1;}else $yes=0;
	    ?>
            <li <?php if($yes==1) echo "class='has_list'";?>><a href="<?php echo $link;?>"><?php echo $top_page['p_title'];?></a><?php if($yes==1) echo $lists;?></li>
            <?php }?>
